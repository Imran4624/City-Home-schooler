...
export const SearchPlace = ({ setLocation, closeModal }) => {
  ...
  const searchLocation = (value) => {
    setText(value)
    const url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${value}&components=country:us|country:pr|country:vi|country:gu|country:mp&types=(regions)&key=${apiKey}`
  ...
  return (
    <View style={{ flex: 1, }}>
      ...
      <FlatList
        style={{ padding: '3%' }}
        data={predictions}
        renderItem={({ item }, index) => <SearchResultCard key={index} prediction={item} onPress={resultPress} />}
...
const SearchResultCard = ({ prediction, onPress, key }) => {
  return (
    <Pressable key={key} onPress={() => onPress(prediction)}>
      <View>
