...
export const NearMe = ({ navigation }) => {
  ...
  const updateLocationResponse = (selectedLocation) => {
    ...
    getUsers(selectedLocation?.geometry.location.lat, selectedLocation?.geometry.location.lat)
    console.log(selectedLocation?.geometry.location.lat, selectedLocation?.geometry.location.lat)
  }

  return (
    <View style={styles.homeContainer}>
      {/* <Text>Near me</Text> */}
      <View style={{ height: deviceWidth - (deviceWidth / 4), width: deviceWidth, backgroundColor: MyTheme.lightBackground }}>
        <MapView
          provider={PROVIDER_GOOGLE} // remove if not using Google Maps
          style={styles.map}
          region={{
            latitude: selectedLocation ? selectedLocation?.geometry?.location?.lat : 37.78825,
            longitude: selectedLocation ? selectedLocation?.geometry?.location?.lng : -122.4324,
            latitudeDelta: 0.15,
            longitudeDelta: 0.121,
          }}
        >
          {usersList.map((marker, index) => !marker?.latitude ? null : (
            <Marker
              key={index}
              coordinate={{
      ...
      <View style={{ flex: 1, width: '100%' }}>
        ...
        {usersList.length > 0 &&
          <FlatList
            style={{ padding: '3%', }}
            data={usersList}
            renderItem={({ item }) => <UserCard user={item} onPress={userPress} />}
            keyExtractor={item => item.id}
          />
        }
        {usersList.length == 0 &&
          <View style={{ justifyContent: 'center', alignItems: 'center' }} >
            <Text style={{ color: MyTheme.textDark, fontSize: 16, }}>
              No User
            </Text>
          </View>
        }
      ...
      <Modal
        ...
        <View style={{ flex: 1, backgroundColor: MyTheme.background }}>
          <SearchPlace setLocation={updateLocationResponse} closeModal={() => setModalVisible(false)} />
        </View>
      </Modal>
